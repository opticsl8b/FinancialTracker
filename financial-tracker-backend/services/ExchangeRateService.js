...
// -- (getTwdUsdtExchangeRate 函式保持不變，已是最新 Bitopro 和 MAX 整合版本) --
// ... (之前的 getTwdUsdtExchangeRate 程式碼) ...
...
async function getCryptoPrices(symbols) {
    ...
    const coingeckoIdMap = {
        ...
        PEPE: 'pepe',
        APT: 'aptos',
        // 'VIRTUAL': 'virtual-coin-id' // 如果有 CoinGecko ID，請在此處添加
    };

    const coingeckoIds = symbols.map(s => coingeckoIdMap[s]).filter(id => id); // 過濾掉沒有 ID 的幣種
    const vsCurrencies = 'usd,twd,usdt'; // 我們想查詢的對比貨幣
    ...
    try {
        ...
        // CoinGecko API 返回的數據以 CoinGecko ID 為鍵，而不是原始符號
        // 我們需要將它們轉換回原始符號，並提取 USD 價格
        for (const symbol of symbols) {
            ...
            if (coingeckoId && response.data[coingeckoId]) {
                prices[symbol] = {
                    ...
                    usdt: response.data[coingeckoId].usdt || null // 這裡 USDT 價格通常直接等於 USD 價格
                };
            } else {
                prices[symbol] = { usd: null, twd: null, usdt: null }; // 未找到價格的幣種設為 null
            }
        }
        
        console.log(`[${new Date().toISOString()}] Crypto Prices Fetched from CoinGecko:`, prices);
        return prices;
